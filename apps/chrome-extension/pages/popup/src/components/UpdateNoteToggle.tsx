...
export const UpdateNoteToggle = ({ updatedVersion }: { updatedVersion: string }) => {
  ...
  return (
    <Toggle
      ...
      <span
        className={`absolute left-1/2 -translate-x-1/2 top-full mt-2 hidden w-max px-2 py-1 text-xs text-white bg-black rounded-md opacity-50 group-hover:block group-hover:opacity-90 transition-opacity ${updatedVersion.length && 'text-red-500'}`}>
        Update note
      </span>
    </Toggle>
  );
