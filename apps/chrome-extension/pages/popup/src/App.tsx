...
//     if(!Object.values){
//       const filteredData = Object.values(tickers).filter(ticker => ticker.maket?.startsWith("KRW-"))
//       return filteredData
//     }
//   },{tickers})
//   const tableDataUpbitBTC = useMemo(()=>{
//     if(!Object.values){
//       const filteredData = Object.values(tickers).filter(ticker => ticker.maket?.startsWith("BTC-"))
//       return filteredData
//     }
//   },{tickers})
...
//     if(!Object.values){
//       const filteredData = Object.values(tickers).filter(ticker => ticker.maket?.startsWith("USDT-"))
//       return filteredData
//     }
//   },{tickers})
//   const tableDataBithumbKRW = useMemo(()=>{
//     if(!Object.values && exchangePlatform ==="bithumb" ){
//       const filteredData = Object.values(tickers).filter(ticker => ticker.maket?.startsWith("KRW-"))
//       return filteredData
//     }
//   },{tickers})
...
//   //즐겨찾기
//   //테이블 table useReactTable bitumbtc, bithubKRW 새로파서
//   //컬럼별 테이블 렌더링 다르게 하는걸로
...
//    const mergedString = platform+marketType
//    let renderTable

//     switch(mergedString){

//       case  "upbitKRW":
//       return renderTable=tableUpbitKRW
//       case "upbitBTC":
//       return renderTable=tableUpbitBTC
//       case "upbitUSDT":
//         return renderTable=tableUpbitUSDT
//       case "bithumbKRW":
//         return renderTable=tableBithumbKRW
//       case "bithumbBTC":
//         return renderTable=tableBithumbBTC
...
//       <TableRow>
//         <TableCell
//           colSpan={renderTable.getAllColumns().filter(col => col.getIsVisible()).length || 1}
...
//     ) : (
//       <>
//         {renderTable.getTopRows().map(row => (
//           <TableRow
...
//           </TableRow>
//         ))}
//         {renderTable.getCenterRows().map(row => (
//           <TableRow
...
const App = () => {
  ...
  const tableDataBithumbUSDT = useMemo(() => {
    if (!Object.values(tickers).length) return fallbackData;
    if (exchangePlatform === 'bithumb') {
      const filtered = Object.values(tickers).filter(ticker => ticker.market?.startsWith('USDT-'));
      console.log('tableDataBithumbUSDT:', filtered);
  ...
  const tableBithumbUSDT = useReactTable({
    data: tableDataBithumbUSDT,
    columns,
    getCoreRowModel: getCoreRowModel(),
    getSortedRowModel: getSortedRowModel(),
    getFilteredRowModel: getFilteredRowModel(),
  ...
  const renderSelectedTable = (platform: 'upbit' | 'bithumb', marketType: 'KRW' | 'BTC' | 'USDT') => {
    ...
    switch (key) {
      ...
      case 'bithumbUSDT':
        if (tableDataBithumbUSDT.length === 0) {
          console.warn('USDT market is not supported for Bithumb, returning empty table');
        }
        return tableBithumbUSDT;
      default:
    ...
    <ThemeProvider defaultTheme="dark" storageKey="como-ui-theme">
      <div className={`flex-col ${!wideSize ? 'w-[420px] h-[430px]' : 'w-[800px] h-[600px]'} overflow-hidden`}>
        ...
        <main
          ...
          {exchangePlatform === 'bithumb' && exchangeMarketType === 'USDT' && tableDataBithumbUSDT.length === 0 && (
            <div className="text-center text-red-500 text-xs p-2">USDT market is not supported for Bithumb.</div>
          )}
        </main>
      </div>
    </ThemeProvider>
